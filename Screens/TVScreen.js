...
function TVScreen({ navigation, route }) {
  ...
  useEffect(() => {
    async function fetchDatas() {
      await axios
        .get(
          `https://api.themoviedb.org/3/tv/${id}?api_key=d0a6b59ce5d4305ca6cf3e0124eeec2f&language=en-US`
        )
        .then((req) => {
          // console.log(req);
    ...
    <View style={{ flex: 1, backgroundColor: "#222" }}>
      ...
      >
        ...
        >
          ...
          <SeasonBanners showDetails={showDetails} />
        </View>
      </ScrollView>
    </View>
  );
